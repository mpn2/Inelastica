version = "SVN $Id: SupercellPhonons.py 434 2017-03-04 20:55:14Z tfrederiksen $"
print version
...
def GetOptions(argv,**kwargs):
    ...
    # TODO: input file with k-points
    #p.add_argument('-k','--kpointfile',dest='kfile',default=None,
    #               help='Input file with electronic k-points to be evaluated [default=High sym directions]')
    ...
    p.add_argument('--sort',dest='sorting',action='store_true',default=True,
    ...
    if "log" in kwargs:
        ...
        options.Logfile = 'ElectronBands.log'

    options.xvfilename = glob.glob('*.XV')
    if len(options.xvfilename)!=1:
        print "*.XV files found:",options.xvfilename
        sys.exit('Only one .XV file should be present.')
    else:
        options.xvfilename=options.xvfilename[0]
    options.tshsfilename = glob.glob('*.TSHS')
    if len(options.tshsfilename)!=1:
        print "*.TSHS files found:",options.tshsfilename
        sys.exit('Only one .TSHS file should be present.')
    else:
        options.tshsfilename=options.tshsfilename[0]

    return options
  

class Supercell():
    def __init__(self,xvfn,tshsfn):
        self.xvfn, self.tshsfn = xvfn, tshsfn
        self.Sym = Symmetry.Symmetry(self.xvfn) # find symmetry
        self.Sym.what() # High symmetry directions 
