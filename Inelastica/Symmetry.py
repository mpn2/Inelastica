version = "SVN $Id: Symmetry.py 408 2016-05-26 11:57:06Z tfrederiksen $"
print version
...
class Symmetry:
    ...
    def symmetrizeFC(self,FC,FCfirst,FClast,radi=0.0):
        ...
        if radi==0.0:
            radi = findRadi(self.pbc[0],self.pbc[1],self.pbc[2])-2*self.accuracy
        ...
        #FCs = FC.copy()
        ifn=0
        wNCDF.write('FC%i.nc'%ifn,FCo,'FC')
        while iter<10 and change>1e-10:
            ...
            for iU in range(NU):
                ...
                print N.max((FCs-FCo)*(N.abs(FCs)>1e-6))
                ifn=ifn+1
                wNCDF.write('FC%i.nc'%ifn,FCs,'FC')
